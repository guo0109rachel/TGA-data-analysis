function Conversion = TGA_conv_Fe3O4(rawdata,dopant,percent)
%percent means the dopant percentage
%Conversion is based on oxygen conversion to Fe3O4
max_weight=max(rawdata);
Weightper=rawdata/max_weight*100;
...
if dopant==0
    Conversion=(100-Weightper)/((O/(3*(O*3+Fe*2)))*100);
    %dopant=1 means La dopant
else if dopant==1
        WeightFe2O3=100*(2*Fe+3*O)/(percent/100*(2*La+3*O)+(O*3+Fe*2));
        Conversion=(100-Weightper)/((O/(3*(O*3+Fe*2)))*WeightFe2O3);
